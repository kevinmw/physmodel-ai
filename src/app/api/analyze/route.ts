...
- 二维碰撞/2D碰撞/斜碰 → 2d_collision
- 滚动滑移/滑动到滚动/保龄球滚动 → rolling_slipping
- 物理摆/杆摆/复摆 → physical_pendulum
- 大角度摆/大振幅单摆/非线性摆 → large_angle_pendulum
- 堆叠球/超级球/多球碰撞 → stacked_ball
- LC电路/LC振荡/电磁振荡电路 → lc_circuit
- 法拉第/电磁感应/楞次定律/感应电流 → faraday_law
- 电容器/充放电/RC电路 → capacitor
- 落上弹簧/弹簧接物/质量落弹簧 → mass_spring_drop
- 其他 → other
...
- 2d_collision: m1(kg), m2(kg), v1(m/s), theta(碰撞角deg), e(恢复系数)
- rolling_slipping: v0(初速度m/s), mu(摩擦系数), R(半径m), g
- physical_pendulum: L(杆长m), theta0(初始角度deg), g
- large_angle_pendulum: L(摆长m), theta0(初始角度deg), g
- stacked_ball: h(下落高度m), g
- lc_circuit: C(电容F), L(电感H), Q0(初始电荷C)
- faraday_law: B(磁感应强度T), v(速度m/s), R(电阻Ω), w(线圈宽度m)
- capacitor: V(电源电压V), R(电阻Ω), C(电容F)
- mass_spring_drop: m(质量kg), k(劲度系数N/m), h(下落高度m), g
...
function refineClassification(analysis: any): string {
  ...
  if (text.includes("二维碰撞") || text.includes("2d碰撞") || text.includes("斜碰")) return "2d_collision";
  if (text.includes("滚动滑移") || (text.includes("滑动") && text.includes("滚动")) || text.includes("保龄球")) return "rolling_slipping";
  if (text.includes("物理摆") || text.includes("杆摆") || text.includes("复摆")) return "physical_pendulum";
  if (text.includes("大角度") && (text.includes("摆") || text.includes("振幅")) || text.includes("非线性摆")) return "large_angle_pendulum";
  if (text.includes("堆叠球") || text.includes("超级球") || text.includes("多球碰撞")) return "stacked_ball";
  if (text.includes("lc电路") || text.includes("lc振荡") || text.includes("lc回路")) return "lc_circuit";
  if (text.includes("法拉第") || text.includes("电磁感应") || text.includes("楞次")) return "faraday_law";
  if (text.includes("电容器") || text.includes("充放电") || text.includes("rc电路")) return "capacitor";
  if (text.includes("落上弹簧") || text.includes("弹簧接") || (text.includes("落下") && text.includes("弹簧"))) return "mass_spring_drop";
  return "other";
}
